#!/usr/bin/env python3
"""Validate claim-page layering for the power-posing pilot."""
...
def validate() -> dict[str, int]:
    ...
    for claim_page_path in claim_pages:
        ...
        claim_meta = load_frontmatter(claim_obj_path)
        claim_title_match = re.search(r"\*\*Title:\*\* (.+?)\s{2,}$", markdown, re.M)
        claim_title = claim_title_match.group(1).strip() if claim_title_match else None
        if claim_title != claim_meta.get("title"):
            errors.append(
                f"{claim_id}: claim page title `{claim_title}` does not match governed object title `{claim_meta.get('title')}`"
            )
...
def main() -> None:
    try:
        ...
        print("Claim page layering failed.", file=sys.stderr)
    ...
    print("Claim page layering passed.")
    print()
