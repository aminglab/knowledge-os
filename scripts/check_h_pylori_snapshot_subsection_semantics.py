#!/usr/bin/env python3
"""Validate snapshot subsection semantic anchors for the h-pylori-ulcer pilot."""
...
def validate() -> dict[str, int]:
    ...
    for section_title, anchors in sorted(anchor_entries.items()):
        ...
        if not section_body:
            errors.append(f"snapshot-v0: missing section body for `{section_title}`")
            continue
        ...
        for anchor in anchors:
            ...
            if normalize_text(anchor) not in normalized_section:
                errors.append(
                    f"snapshot-v0: section `{section_title}` is missing required anchor `{anchor}`"
                )
...
def main() -> None:
    try:
        ...
        print("H. pylori snapshot subsection semantic anchors failed.", file=sys.stderr)
    ...
    print("H. pylori snapshot subsection semantic anchors passed.")
    print()
