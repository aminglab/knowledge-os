...
function buildResultCard(trigger, object) {
  if (trigger === 'scan missing evidence') {
    return {
      title: 'Missing-evidence scan',
      outputClass: 'scan_result',
      anchor: object.label,
      summary: 'The current surface shows a bounded missing-evidence scan rather than a claim rewrite or verdict action.',
      basis: ['current object anchor remains explicit','current reading still depends on bounded case surfaces'],
      unresolved: ['scan result does not itself create evidence','stronger basis still requires later governed work'],
      nextStep: ['inspect current support gaps','keep the scan result in draft-only posture'],
      audit: 'Generated from a bounded scan trigger. No evidence object was created.'
    };
  }
  if (trigger === 'draft dissent response') {
    return {
      title: 'Dissent-response draft',
      outputClass: 'draft_response',
      anchor: object.label,
      summary: 'The cockpit can now stage a bounded draft response posture without claiming that the dissent has been resolved.',
      basis: ['current pressure surface remains visible','draft posture stays downstream of the current case semantics'],
      unresolved: ['draft response is not a verdict action','further basis may still be required'],
      nextStep: ['compare draft response with current pressure','keep non-resolution honesty explicit'],
      audit: 'Generated from a bounded response-draft trigger. No dissent object was resolved.'
    };
  }
  ...
  if (trigger === 'prepare progress summary') {
    return {
      ...
      anchor: object.label,
      summary: 'The cockpit currently supports bounded state switching, bounded lens switching, bounded object browsing, and expanded structured result-card coverage.',
      basis: ['interactive floor already passed','first result-card lift already passed'],
      unresolved: ['trigger-result coverage is still intentionally incomplete','runtime remains out of scope'],
      nextStep: ['extend only if current honesty remains intact','do not flatten current holds'],
...
function renderResult(){
  if(!resultCard){ resultSurface.innerHTML=`<div class="result-empty"><strong>No structured draft result yet.</strong><p>Run one of the four covered triggers to render an expanded structured result card. <code>suggest canonical continuation</code> intentionally remains outside current structured-card coverage.</p></div>`; return; }
